...
def check_active_segment(current_datetime, start_dates, durations, activity_arrays):
    for start_date, duration, activity_array in zip(start_dates, durations, activity_arrays):
        end_date = start_date + duration  # No need to subtract 1 day

        if start_date <= current_datetime <= end_date:
            current_time = current_datetime.time()
            current_day_index = calculate_day_index(current_datetime, start_date)
            print(current_day_index)  # Print the current day index for debugging

            # Check if the current activity_array value is 4 and the current time is within the specified ranges
            if activity_array[current_day_index] == 4 and (800 <= current_time.hour * 100 + current_time.minute < 9.30  or 1430 <= current_time.hour * 100 + current_time.minute <= 1600):
...
while True:
    ...
    if check_active_segment(current_datetime, start_dates, durations, activity_arrays):
        for digit in pattern1:
             ...
             #  break
            control_leds(digit)
            #if current_datetime.time() > dt_time(9, 30) or current_datetime.time() >= dt_time(16, 0):
                #break
